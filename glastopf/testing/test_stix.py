...
class Test_Stix(unittest.TestCase):
    ...
    def test_stix_transform(self):
        ...
        test_event.http_request = HTTPHandler(
            http_request_content, None, server_version="", sys_version=""
        )
        ...
        ) = self.xml_validator.validate(StringIO(stix_package_xml.encode("utf-8")))
        self.assertTrue(
            isvalid, "Error while parsing STIX xml: {0}".format(validation_error)
        )
        self.assertTrue(
            "<HTTPSessionObj:User_Agent>test</HTTPSessionObj:User_Agent>"
            in stix_package_xml
        )
        self.assertTrue(
        ...
        self.assertTrue(
            "<HTTPSessionObj:Value>/test</HTTPSessionObj:Value>" in stix_package_xml
        )
        self.assertTrue(
    ...
    def test_stix_transform_event_with_rfidata(self):
        ...
        rfi_md5 = hashlib.md5(str.encode(rfi_data)).hexdigest()
