    ...
    def get_comments(self, ip_address):
        ...
        if driver == "sqlite":
            ...
            try:
                ...
                c = connection.cursor()
                c.execute(
                    """SELECT comments FROM ip_profiles WHERE ip==?""", (ip_address,)
                )
        ...
        if driver == "sqlite":
            ...
            try:
                ...
                if len(comments_list) == 0:
                    c.execute(
                        """INSERT INTO ip_profiles(ip, total_requests, total_scans, avg_scan_duration, scan_time_period, comments) VALUES (?, ?, ?, ?, ?, ?)""",
                        (ip_address, 0, 0, 1, 1, comment),
                    )
                else:
                    ...
                    for i in range(len(comments_list)):
                        ...
                        c.execute(
                            """UPDATE ip_profiles SET comments=(?) WHERE ip==?""",
                            (comments, ip_address),
                        )
                connection.commit()
                c.close()
                self.logger.info(
                    "Comment added to database for ip address {0}".format(ip_address)
                )
            except sqlite3.DatabaseError as e:
