...
class StixTransformer(object):
    ...
    def transform(self, event):
        ...
        jinja_vars = {
            ...
            "incident_timestamp": datetime.strptime(
                event.event_time, "%Y-%m-%d %H:%M:%S"
            ).isoformat(),
            "glastopf_version": glastopf.__version__,
            "include_contact_info": self.config.getboolean(
                "taxii", "include_contact_info"
            ),
        ...
        if event.file_name is not None:
            with file(
                os.path.join(self.files_dir, event.file_name), "r"
            ) as file_handler:
    ...
    def _get_parsed_header(self, event):
        ...
        for header_name in event.http_request.headers:
            ...
            if header_name_cap in CYBOX_HEADERS:
                parsed_header.append(
                    (header_name_cap, event.http_request.headers.get(header_name, ""))
                )
