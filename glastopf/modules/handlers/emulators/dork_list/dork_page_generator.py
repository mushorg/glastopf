...
class DorkPageGenerator(object):
    ...
    def prepare_text(cls):
        ...
        with codecs.open(text_file, "r", "utf-8") as text_file:
            for text_line in text_file.readlines():
                ...
                if text_line != "":
                    line_list.append(
                        unicodedata.normalize("NFKD", text_line).encode(
                            "ascii", "ignore"
                        )
                    )
        return line_list
    ...
    def generate_dork_pages(self):
        ...
        if len(inurl_list) < INURL_MIN_SIZE:
            dork_seeds = random.sample(
                self.database.get_dork_list("inurl"), INURL_MIN_SIZE
            )
        ...
        for count_new_pages in range(5):
            ...
            dork_page = self.surface_creator.get_index(
                choice(intitle_list), "/index", body, "Footer Powered By"
            )
            page_md5 = hashlib.md5(dork_page).hexdigest()
            new_pages.append(page_md5)
            with codecs.open(
                "{0}/{1}".format(self.pages_path, page_md5), "w", "utf-8"
            ) as dork_file:
